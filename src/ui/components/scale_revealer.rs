...
mod imp {
    ...
    pub(crate) struct ScaleRevealer {
    ...
    impl ObjectSubclass for ScaleRevealer {
        const NAME: &'static str = "PaplScaleRevealer";
    ...
    impl WidgetImpl for ScaleRevealer {
        fn snapshot(&self, snapshot: &gtk::Snapshot) {
            ...
            if progress > 0.0 {
                // We are in the middle of the transition, so do the cross fade
                // transition.
                ...
                // We are at the beginning of the transition, show the source
                // widget.
                source_widget_texture.snapshot(snapshot, obj.width().into(), obj.height().into());
            }
        }
    ...
    impl ScaleRevealer {
        ...
        fn set_reveal_child(&self, reveal_child: bool) {
            ...
            if reveal_child {
                ...
                if let Some(source_widget) = self.source_widget.upgrade() {
                    // Render the current state of the source widget to a
                    // texture. This will be needed for the transition.
                    let texture = render_widget_to_texture(&source_widget);
                    self.source_widget_texture.replace(texture);

                    // Hide the source widget.
                    // We use opacity here so that the widget will stay
                    // allocated.
        ...
        fn set_source_widget(&self, source_widget: Option<&gtk::Widget>) {
            ...
            // The widget of a transition that is interrupted by a new one is
            // shown again, since it won't be restored when the new transition
            // ends.
            if let Some(previous_widget) = self.source_widget.upgrade() {
                previous_widget.set_opacity(1.0);
            }
...
glib::wrapper! {
    /// A widget to reveal a child with a scaling animation.
    pub(crate) struct ScaleRevealer(ObjectSubclass<imp::ScaleRevealer>)
